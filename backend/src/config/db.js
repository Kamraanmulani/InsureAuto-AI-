...
const seedInitialUsers = async () => {
  try {
    const admin = await User.findOne({
      $or: [{ email: "admin@insureauto.ai" }, { email: "admin@claimsight.internal" }, { role: "ADMIN" }]
    });
    if (!admin) {
    ...
    const assessor = await User.findOne({
      $or: [{ email: "assessor@insureauto.ai" }, { email: "assessor@claimsight.internal" }]
    });
    if (!assessor) {
  ...
  try {
    const claims = await Claim.find({
      $or: [
        ...
        { status: { $in: ['PENDING', 'PROCESSED', 'REVIEW_REQUIRED', 'REVIEWED'] } },
    ...
    for (const claim of claims) {
      ...
      if (!claim.evidence || claim.evidence.length === 0) {
        const fileRef = claim.primaryAnnotatedKeyframeUrl || claim.annotatedImagePath || 'evidence_file';
        claim.evidence = [{
      ...
      if (!claim.aiAssessment || !claim.aiAssessment.recommendation) {
        claim.aiAssessment = {
          ...
          scores: claim.decision?.scores || { damage: 0, fraud: 0, consistency: 0 },
      ...
      if (!claim.decision || !claim.decision.outcome) {
        claim.decision = {
          outcome: ['APPROVED', 'REJECTED', 'CLOSED'].includes(claim.status) ? claim.status : 'PENDING',
      ...
      if (!claim.auditHistory || claim.auditHistory.length === 0) {
        claim.auditHistory = [{
          ...
          actor: { id: 'SYSTEM_MIGRATION', name: 'Database Normalizer', role: 'SYSTEM' },
          previousStatus: 'LEGACY',
          newStatus: claim.status,
          details: 'Claim data safely migrated to normalized motor claim workflow model'
        }];
      }
